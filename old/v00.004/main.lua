...
--			local dust = 0.5 + math.random()/2
	...
	buttons = {}
	buttons['reset'] = {text = 'reset', x=text_shift + 120, y=10, w=35, h=15}
--	slider = {x = }
...
function love.draw()
	...
	love.graphics.print ('p x:' .. x .. ' y:' .. y .. ' d:' .. string.format("%0.2f", p.d) .. ' dust:' .. string.format("%0.2f", dust), text_shift + 120, 25)
	...
	if 	(m_x >= (f.xmin-f.step/2)) and (m_x <= f.xmax+f.step/2) and 
		...
		local dust = map[y] and map[y][x] or 0
		love.graphics.print ('c pos x:' .. x .. ' y:' .. y .. ' dust:' .. string.format("%0.2f", dust), text_shift + 120, 40)
	end
	
	-- button
	my.button_draw {x=text_shift + 120, y=10, w=35, text = 'reset'}
